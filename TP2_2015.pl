...
listeActeurs(L) :- findall(X, acteur(X,_,_,_,_), L).
...
experience(IdAct,date(Annee,_,_),Ne) :- acteur(IdAct,_,_,date(Annee1,_,_),_), Ne is Annee-Annee1.
...
filtreCritere([],_).
filtreCritere([X|Lc], ActId) :- critere(X, ActId), filtreCritere(Lc, ActId).
...
totalSalaireMin([],0).
totalSalaireMin([A|LActeur],Total) :- acteur(A,_,T1,_,_), totalSalaireMin(LActeur,T2), Total is T1+T2.
...
selectionActeursCriteres(Lcriteres,Lacteurs) :- findall(Act,filtreCritere(Lcriteres,Act),Lacteurs).
...
selectionActeursCriteresNouovelle([],_,_).
...
selectionActeursFilm(IdFilm,Lacteurs) :- findall(Nom,(acteur(Nom,_,_,_,X),restriction(X,IdFilm)),Lacteurs).
...
affectationDesRolesSansCriteres(IdFilm) :- selectionNActeursFilm2(IdFilm, L), not(L = pasAssezDacteur), doAffectationDesRoles(IdFilm, L), !.
...
affectationDesRoles(IdFilm, LCriteres) :- affectationDesRolesCriteres(IdFilm, LCriteres, LChoisis), doAffectationDesRoles(IdFilm, LChoisis), !.
...
produire(NomMaison,IdFilm):- film(IdFilm,Type,Realisateur,Producteur,Cout,Duree,NActeur,Budget),
                                 Producteur == 'pasDeProducteur',
                                 realisateur(Realisateur,_),
                                 ...
                                 retract(film(Id,Type,Realisateur,Producteur,Cout,Duree,NActeur,Budget)),
                                 assert(film(Id,Type,Realisateur,NomMaison,Cout,Duree,NActeur,Budget)).
...
plusieursFilms(N, Lacteurs) :- findall(Nom, (selectionFilm(Nom, LFilms), length(LFilms, X), X >= N), Lacteurs), length(Lacteurs, Lacteur2), Lacteur2 > 0.
selectionFilm(Lacteurs, Films) :- setof(Film, joueDans(Lacteurs, Film), Films).
